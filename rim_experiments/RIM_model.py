...
class RIM(object):
    ...
    def test_dae(self, config):
        ...
        for t in range(0,batch_idxs):
            ...
            print("[RIM] Saving voxel parts as .npy...")

            for x in range(1, self.nlevels + 1):
                print(f"[RIM]  Level {x}, parts = {2**x}")
                for split in range(2**x):
                    voxel_part = model_float[x][:, :, :, split]
                    save_path = os.path.join(
                    config.result_dir, f"t{t}_vox_level{x}_part{split}.npy")
                    np.save(save_path, voxel_part)
                    print("   saved:",
            save_path,
            "shape:",
            voxel_part.shape,
